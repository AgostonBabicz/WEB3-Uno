...
describe('create round from valid memento', () => {
  ...
  it('reads the discard pile from the memento', () => {
    expect(
      is({ type: 'NUMBERED', color: 'BLUE', number: 7 })(round.discardPile().top()),
    ).toBeTruthy()
  ...
  describe('direction from memento', () => {
    ...
    it('proceeds to the next lower-numbered player (or highest-numbered) otherwise', () => {
      const memento = {
        players: ['A', 'B', 'C'],
        hands: [
          [{ type: 'WILD' }, { type: 'DRAW', color: 'YELLOW' }],
          [{ type: 'REVERSE', color: 'GREEN' }],
          [{ type: 'SKIP', color: 'RED' }],
        ],
        drawPile: [{ type: 'WILD DRAW' }],
...
describe('create finished round from memento', () => {
  ...
  it("doesn't need a playerInTurn if the game is finished", () => {
    const hands = [[], [{ type: 'REVERSE', color: 'GREEN' }], [{ type: 'SKIP', color: 'RED' }]]
    expect(
      createRoundFromMemento({ ...memento, hands, playerInTurn: undefined }).playerInTurn(),
    ).toBeUndefined()
  })
})
  ...
  it('throws less than 2 players', () => {
    expect(() =>
      createRoundFromMemento({
        ...memento,
        players: ['A'],
        hands: [[{ type: 'WILD' }, { type: 'DRAW', color: 'YELLOW' }]],
      }),
    ).toThrowError()
  })
